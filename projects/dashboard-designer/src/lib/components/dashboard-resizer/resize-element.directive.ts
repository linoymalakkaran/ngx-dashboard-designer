...
export class DashResizeElementDirective implements OnChanges, OnDestroy {
  ...
  public listenMouseDownEvent(): void {
    ...
    this.mouseClickListener = this.renderer2.listen(
      this.elementRef.nativeElement,
      event,
      evt => this.onMouseDown(evt)
    );
  }
  ...
  public onMouseDown(evt: MouseEvent): void {
    ...
    this.mouseUpListener = this.renderer2.listen('document', 'mouseup', event =>
  ...
  private get targetNativeElement(): HTMLElement {
    return this.targetElement instanceof ElementRef
  ...
  private generateValuesForEvent(evt: MouseEvent): DashResizeElementEvent {
    ...
    if (this.rect) {
      if (currentTopValue < this.rect.top) {
        currentHeightValue =
          this.targetElementHeightValue +
          this.targetElementTopValue -
          this.rect.top;
      ...
      if (currentLeftValue < this.rect.left) {
        currentWidthValue =
          this.targetElementWidthValue +
          this.targetElementLeftValue -
          this.rect.left;
