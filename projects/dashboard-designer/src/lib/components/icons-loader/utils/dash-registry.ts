 ...
 * @docs-private
 */
export function getDashIconNameNotFoundError(iconName: string): Error {
  return Error(`Unable to find icon with the name "${iconName}"`);
}

/**
 * Returns an exception to be thrown when the consumer attempts to use
 * `<dash-icon>` without including @angular/common/http.
 * @docs-private
 */
export function getDashIconNoHttpProviderError(): Error {
  return Error(
    'Could not find HttpClient provider for use with Angular Dasherial icons. ' +
 ...
 * @docs-private
 */
export function getDashIconFailedToSanitizeUrlError(
  url: SafeResourceUrl
): Error {
  return Error(
    `The URL provided to DashIconRegistry was not trusted as a resource URL ` +
 ...
 * @docs-private
 */
export function getDashIconFailedToSanitizeLiteralError(
  literal: SafeHtml
): Error {
  return Error(
    `The literal provided to DashIconRegistry was not trusted as safe HTML by ` +
...
/**
 * Service to register and display icons used by the `<dash-icon>` component.
...
export class DashIconRegistry implements OnDestroy {
  ...
  /**
   * The CSS classes to apply when an `<dash-icon>` component has no icon name, url, or font
   * specified. The default 'material-icons' value assumes that the material icon font has been
   */
  private _defaultFontSetClass = ['material-icons', 'dash-ligature-font'];
  ...
  /**
   * Sets the CSS classes to be used for icon fonts when an `<dash-icon>` component does not
  ...
  /**
   * Returns the CSS classes to be used for icon fonts when an `<dash-icon>` component does not
   * have a fontSet input value, and is not loading an icon by name or URL.
   */
  ...
  getNamedSvgIcon(
    ...
    return observableThrow(getDashIconNameNotFoundError(key));
  }
  ...
  private _getSvgFromIconSetConfigs(
    ...
    const iconSetFetchRequests: Observable<TrustedHTML | null>[] =
      iconSetConfigs
    ...
    return forkJoin(iconSetFetchRequests).pipe(
      map(() => {
        ...
        if (!foundIcon) {
          throw getDashIconNameNotFoundError(name);
        }
  ...
  private _fetchIcon(iconConfig: SvgIconConfig): Observable<TrustedHTML> {
    ...
    if (!this._httpClient) {
      throw getDashIconNoHttpProviderError();
    }
    ...
    // TODO: add an ngDevMode check
    if (!url) {
      throw getDashIconFailedToSanitizeUrlError(safeUrl);
    }
...
function isSafeUrlWithOptions(
  value: any
): value is SafeResourceUrlWithIconOptions {
