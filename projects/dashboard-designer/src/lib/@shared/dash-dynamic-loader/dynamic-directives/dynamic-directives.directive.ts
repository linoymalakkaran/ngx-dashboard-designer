...
@Directive({
  selector: '[dashDynamicDirectives],[ngComponentOutletDashDynamicDirectives]',
  providers: [IoFactoryService]
})
export class DynamicDirectivesDirective implements OnDestroy, DoCheck {
  @Input()
  dashDynamicDirectives: DynamicDirectiveDef<any>[];
  @Input()
  ngComponentOutletDashDynamicDirectives: DynamicDirectiveDef<any>[];

  @Output()
  dashDynamicDirectivesCreated = new EventEmitter<DirectiveRef<any>[]>();

  private lastCompInstance: any;

  private get directives() {
    return (
      this.dashDynamicDirectives || this.ngComponentOutletDashDynamicDirectives
    );
  }
  ...
  private processDirChanges(
    ...
    if (createdDirs.length) {
      this.dashDynamicDirectivesCreated.emit(createdDirs.filter(Boolean));
    }
  }
